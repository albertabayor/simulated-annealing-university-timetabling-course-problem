...
const DAY_LABELS = ["Mon", "Tue", "Wed", "Thu", "Fri"]; // Sen–Friday
...
const SHEET_GUESS_ORDER = ["Lesson", " Contoh lesson", "Courses", "Mata Kuliah", "Matakuliah", "Kuliah", "Sheet1"];
const HEADER_MAP_CANDIDATES = {
  courseId: ["courseId", "kode", "kode mk", "kode_mk", "kode matakuliah", "id", "id mk", "id_mk"],
  name: ["name", "mata kuliah", "matakuliah", "nama mk", "nama matakuliah", "judul", "mk"],
  class: ["class", "kelas", "rombel", "group", "kelompok"],
  lecturer: ["lecturer", "dosen", "pengajar", "nama dosen", "kode dosen"],
  ...
  preferredRoom: ["preferred room", "prefer room", "ruang prefer", "prefer_ruang", "ruangan"],
};
...
function parseExcel(filePath) {
  ...
  const courses = raw
    .map((r) => {
      ...
      if (headerMap.duration) {
        ...
        if (!isNaN(num) && num > 0) {
          // SKS: 1 SKS = 2 slots (100 menit)
          durationSlots = Math.max(1, Math.round(num * 2));
        }
      }
  ...
  // Parse Rooms dari sheet "Ruang"
  let rooms = DEFAULT_ROOMS.map((r, i) => ({ id: i + 1, ...r }));
  if (wb.SheetNames.includes("Ruang")) {
    try {
      const roomSheet = XLSX.utils.sheet_to_json(wb.Sheets["Ruang"], { defval: "" });
      if (roomSheet.length > 0) {
        rooms = roomSheet.map((r, i) => {
          const code = r["Kode Ruangan"] || r["kode"] || r["id"] || `R${i + 1}`;
          const name = r["Nama Ruangan"] || r["nama"] || code;
          const type = name.toLowerCase().includes("lab") ? "lab" : "theory";
          return {
            id: i + 1,
            name: code,
            fullName: name,
      ...
      console.warn("Gagal parse sheet Ruang, pakai default rooms:", e.message);
    }
  }
